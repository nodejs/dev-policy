...
The stability of the Node.js Build process and infrastructure is critical for those who rely on the ability to build Node.js from source as part of their deployment workflow. The ability to support Node.js on multiple platforms requires that Collaborators avoid making breaking changes that require new or updated compiler versions, new C/C++ language level features, etc, without careful consideration and appropriate review.

Pull Requests that introduce new or updated build dependencies that are not currently supported by *Primary* platforms (those supported by the most recent LTS Release), or that cause backwards incompatible modifications to the build environment should be reviewed and approved by the TSC. Note that the TSC may choose to delegate review to a Working Group chartered to handle the Build environment.

Here, "backwards incompatible" is considered relative to the platforms supported by the most recent LTS Release. For example, if the most recent LTS release supports a certain level of the Linux kernel on PowerPC Architecture, and a Pull Request causes the build to break on that specific platform configuration (i.e. by requiring a newer unsupported version of gcc for instance), then it would be considered a backwards incompatible change that should be given additional review and consideration prior to landing.

Smaller changes to the build that can be made without breaking compatibility with *Primary* platforms can be made with appropriate review just like all other Pull Requests.
